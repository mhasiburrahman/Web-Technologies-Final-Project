...
if (isset($_REQUEST["submitReg"])) {
    ...
    if ($hasError == 0) {
        $mydb=new db();
        $myconn=$mydb->opencon();
        $mydb->insertbuyer($full_name,$customer_email,$customer_username,$customer_number,$customer_age,$customer_password,$Customer_confirm_password,$customer_gender,"buyer",$myconn);
    }
    //     $customer_data = array(
    //         'Full_name' => $full_name,
    ...
    //     //json work
    //     $existing_Data = file_get_contents('../data/buyer_data.json');
